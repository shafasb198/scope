            ...
            <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
              ...
              <a class="dropdown-item preview-item">
                <div class="preview-item-content">
                  <p>
                  <p>Halaman ini berisi detail soal dan<br>jawaban kuis yang telah dibuat</p>
                  </p>
                </div>
              </a>
              <a class="dropdown-item preview-item">
                <div class="preview-item-content">
                  <p>
                  <p>Jawaban yang benar diberi tanda <br>ceklis hijau</p>
                  <img style="width:30%;" src="../../../images/panduan/benar.png">
                  </p>
                </div>
              </a>
              <a class="dropdown-item preview-item">
                <div class="preview-item-content">
                  <p>
                  <p>Jawaban yang salah diberi tanda <br>silang merah</p>
                  <img style="width:30%;" src="../../../images/panduan/salah.png">
                  </p>
                </div>
              </a>
            </div>
        ...
        <table style="width:100%;">
          <tr>
            <td style="vertical-align: middle;">
              <h3 class="font-weight-bold">Detail Kuis <?php echo $data['nama_materi'];?></h3><br>
            </td>
          </tr>
        </table><br>
          <style>
          ...
          <div class="col-md-12 grid-margin stretch-card">
            ...
            <div class="card-body">
            <?php
            $getsoal = "SELECT tb_soal.id_soal ,tb_soal.isi_soal FROM tb_kuis JOIN tb_materi
            ON tb_kuis.id_materi=tb_materi.id_materi 
            JOIN tb_soal ON tb_soal.id_kuis=tb_kuis.id_kuis
            JOIN tb_opsi ON tb_opsi.id_soal=tb_soal.id_soal
            WHERE tb_materi.id_materi='".$_GET["id_materi"]."'
            ...
            $hasilsoal = mysqli_num_rows($eksekusi);                       
            
            
            $i=1;
            if($hasilsoal>0){
              while($soal = mysqli_fetch_array($eksekusi)){
                echo "<h4 class='card-title'>Soal ".$i."</h4>
                <label for='exampleInputUsername1'>".$soal['isi_soal']."</label>";
                  
                  $getbenar = "SELECT tb_opsi.opsi ,tb_opsi.status FROM tb_kuis JOIN tb_materi
                  ON tb_kuis.id_materi=tb_materi.id_materi 
                  JOIN tb_soal ON tb_soal.id_kuis=tb_kuis.id_kuis
                  JOIN tb_opsi ON tb_opsi.id_soal=tb_soal.id_soal
                  WHERE tb_soal.id_soal='".$soal["id_soal"]."' AND tb_opsi.status='Benar'";
                  ...
                  if($hasilbenar>0){
                    while($benar = mysqli_fetch_array($eksbenar)){
                      ...
                      echo "<label class='badge badge-success'><i class='ti-check'></i></label>";
                      echo "</td><td><label for='exampleInputUsername1'>".$benar['opsi']."</label></td></tr>";
                      
                    }
                  }

                  $getsalah = "SELECT tb_opsi.opsi ,tb_opsi.status FROM tb_kuis JOIN tb_materi
                  ON tb_kuis.id_materi=tb_materi.id_materi 
                  JOIN tb_soal ON tb_soal.id_kuis=tb_kuis.id_kuis
                  JOIN tb_opsi ON tb_opsi.id_soal=tb_soal.id_soal
                  WHERE tb_soal.id_soal='".$soal["id_soal"]."' AND tb_opsi.status='Salah'";
                  $ekssalah = mysqli_query($conn, $getsalah);
                  $hasilsalah = mysqli_num_rows($ekssalah);                       
                  
                  if($hasilsalah>0){
                    while($salah = mysqli_fetch_array($ekssalah)){
                      echo "<tr><td>";
                      echo "<label class='badge badge-danger'><i class='ti-close'></i></label>";
                      echo "</td><td><label for='exampleInputUsername1'>".$salah['opsi']."</label></td></tr>";
                    }
                  }
            ...
            ?>
                  ...
                  <a href="../detailmateri.php?id_materi=<?php echo $_GET["id_materi"];?>">
                    <button type="submit" name="btn_batalkan" class="btn btn-outline-primary mr-2">Kembali</button>
          </a>
